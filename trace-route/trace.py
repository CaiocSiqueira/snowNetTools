...
def main():
    ...
    parser.add_argument('--alt', action='store_true', help='Usar protocolos alternativos quando o protocolo padrão (UDP) não funcionar')
    parser.add_argument('--ttl', type=int, default=MAX_TTL, help=f'Definir um número de TTL diferente do padrão ({MAX_TTL})')
    args = parser.parse_args()

    trace_route(args.destino, args.ttl)

def trace_route(destino, time_to_live=MAX_TTL):
    nos = [0]
    ...
    for TTL in range(1, time_to_live + 1):
        ...
        try:
            ...
            print(tabulate.tabulate([['IP', 'TTL', 'PROTOCOLO', 'LOCALIZAÇÃO'], [addr[0], TTL, 'UDP', localizacao(addr[0])]], tablefmt='heavy_grid'))
            nos.append(addr[0]) 
            ...
            try:
                buffer, addr = socRecebe.recvfrom(1024)
                print(tabulate.tabulate([['IP', 'TTL', 'PROTOCOLO', 'LOCALIZAÇÃO'], [addr[0], TTL, 'ICMP', localizacao(addr[0])]], tablefmt='heavy_grid'))
                nos.append(addr[0]) 
